...
export default function StudentInjuryDetailPage() {
    ...
    <div className="px-4 pt-6 pb-4 space-y-5">
      ...
      {injury.trainerNotes && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
          ...
          <p className="text-accent-primary text-xs font-semibold uppercase mb-1">Nota del entrenador</p>
          <p className="text-[#ccc] text-sm">{injury.trainerNotes}</p>
        </motion.div>
      )}
            ...
            {injury.comments.map(c => (
              <div key={c.id} className="rounded-xl p-3"
                ...
                <p className="text-xs font-bold" style={{ color: c.authorRole === "trainer" ? "#00FF87" : "#60a5fa" }}>
                  {c.authorName} {c.authorRole === "trainer" ? "• Entrenador" : "• Tú"}
                </p>
